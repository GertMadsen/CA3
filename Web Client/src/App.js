...
export default class App extends Component {
  ...
  setBookingAvailable = (boolean) => { this.setState({ bookingBoolean: boolean }) }
  setDateStart = (date) => { this.setState({ startDate: date }) }
  setDateEnd = (date) => { this.setState({ endDate: date }) }
  setDateURL = (url) => { this.setState({ dateURL: url }) }
  setLocationURL = (url) => { this.setState({ locationURL: url }) }
  setCategoryURL = (url) => { this.setState({ categoryURL: url }) }
  setLocValue = (value) => { this.setState({ locValue: value }) }
  setCatValue = (value) => { this.setState({ catValue: value }) }
  setReturnURL = (url) => { this.setState({ returnURL: url }) }
  setCar = (car) => { this.setState({ car: car }) }
  setUserFname = (value) => this.setState(prevState => ({ user: { ...prevState.user, firstname: value } }))
  setUserLname = (value) => this.setState(prevState => ({ user: { ...prevState.user, lastname: value } }))
  setUserEmail = (value) => this.setState(prevState => ({ user: { ...prevState.user, email: value } }))
  render() {
    var combiURL = "";
    ...
    if (this.state.dateURL !== "") {
      ...
      if (locStr !== "") { combiURL += locURL; }
      if (catStr !== "") { combiURL += catURL; }
    } else {
      if (locStr !== "") {
        ...
        if (catStr !== "") { combiURL += dateDummy + catURL; }
      }
    }
    return (
      <div>
